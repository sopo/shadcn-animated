...
describe("addComponents", () => {
  ...
  it("rejects unknown components with the available names", async () => {
    ...
    ).rejects.toThrow("Available components: button, checkbox, select, sheet, tabs")
  })
})
...
async function createProject(options: ProjectOptions = {}): Promise<string> {
  const project = await mkdtemp(path.join(os.tmpdir(), "shadcn-animated-test-"))
  ...
  await writeFile(
    ...
    JSON.stringify({ imports: options.packageImports, name: "fixture", private: true }),
  )
  await writeFile(
